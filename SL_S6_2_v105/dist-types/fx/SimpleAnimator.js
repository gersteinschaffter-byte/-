import { Tween, TweenRunner, easeOutCubic, easeOutBack, easeInCubic, linear, } from './Tween';
/** 帧数常量（60fps 基准） */
const FRAMES = {
    fadeIn: 20,
    fadeOut: 15,
...
export default class SimpleAnimator {
     ...
     */
    static fadeIn(target, duration = FRAMES.fadeIn, delay = 0, runner, ease = easeOutCubic) {
     ...
     */
    static fadeOut(target, duration = FRAMES.fadeOut, delay = 0, runner, ease = easeInCubic, hideOnComplete = true) {
        const r = runner ?? new TweenRunner();
        r.add(Tween.delayed(target, { alpha: 0 }, duration, ease, delay, () => { if (hideOnComplete)
     ...
     * @param delay     延迟帧数
     */
    static slideIn(target, direction = 'right', distance = 80, duration = FRAMES.slideIn, delay = 0, runner, ease = easeOutCubic) {
        const r = runner ?? new TweenRunner();
     ...
     * 向指定方向滑出（位移 + 淡出）
     */
    static slideOut(target, direction = 'left', distance = 80, duration = FRAMES.slideOut, delay = 0, runner, ease = easeInCubic, hideOnComplete = true) {
        const r = runner ?? new TweenRunner();
        const toProps = { alpha: 0 };
        switch (direction) {
            case 'left':
        ...
        r.add(Tween.delayed(target, toProps, duration, ease, delay, () => { if (hideOnComplete)
            target.visible = false; }));
        return r;
    }
    // ── 弹入（带回弹） ──────────────────────────────────
     ...
     * 弹入：scale 从 0 → 1 + 回弹缓动
     */
    static popIn(target, duration = FRAMES.popIn, delay = 0, runner) {
        const r = runner ?? new TweenRunner();
        target.alpha = 0;
        ...
        // 用 Tween + 手动同步
        r.add(Tween.delayed(proxy, { scaleX: 1, scaleY: 1, alpha: 1 }, duration, easeOutBack, delay, undefined, undefined));
        // 记住需要在 update 中手动同步 proxy → target
        // 我们用一个特殊的 wrapper tween 来做
        const syncTween = Tween.delayed({ _progress: 0 }, { _progress: 1 }, duration + delay + 2, linear, 0, undefined, undefined);
        // 覆盖 update 来同步
     ...
     */
    static scaleIn(target, duration = FRAMES.scaleIn, delay = 0, runner) {
        const r = runner ?? new TweenRunner();
        const proxy = { scaleX: 0.85, scaleY: 0.85, alpha: 0 };
        target.scale.set(0.85);
        target.alpha = 0;
        target.visible = true;
        r.add(Tween.delayed(proxy, { scaleX: 1, scaleY: 1, alpha: 1 }, duration, easeOutCubic, delay));
        const syncTween = Tween.delayed({ _p: 0 }, { _p: 1 }, duration + delay + 2, linear, 0);
     ...
     * );
     */
    static stagger(targets, animFn, staggerDelay = FRAMES.staggerGap, runner) {
        const r = runner ?? new TweenRunner();
     ...
     */
    static crossFade(oldRoot, newRoot, duration = 12, runner, onMidPoint, onComplete) {
        const r = runner ?? new TweenRunner();
     ...
     */
    static breathe(target, property = 'alpha', min = 0.7, max = 1.0, speed = 2.0) {
        let elapsed = 0;
        return (dt) => {
