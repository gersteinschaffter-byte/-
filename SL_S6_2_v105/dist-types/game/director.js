...
export function getDirectorApiKey() {
    try {
...
export function setDirectorApiKey(key) {
    try {
        if (!key)
            sessionStorage.removeItem(DIRECTOR_SESSION_KEY);
        else
            sessionStorage.setItem(DIRECTOR_SESSION_KEY, key);
    }
    catch (_) { }
}
// -------------------------
...
function pickLocal(stage) {
    // deterministic-ish
    const idx = Math.abs((stage * 9301 + 49297) % LOCAL_TEMPLATES.length);
    return JSON.parse(JSON.stringify(LOCAL_TEMPLATES[idx]));
}
// -------------------------
...
function estimateDirectivePower(mods) {
    let score = 0;
    for (const m of mods) {
        switch (m.type) {
            case 'enemy_hp_mult':
...
export function sanitizeDirective(raw, stage) {
    ...
    const finalRisk = Math.max(risk, normalizedRisk);
...
export function pickLocalDirective(stage) {
    return pickLocal(stage);
}
