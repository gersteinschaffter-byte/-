...
function fighter(id, name, side, hp, atk, def, spd, skills = [], element) {
...
class BattleScene extends BaseScene {
    ...
    constructor(game) {
        ...
        Object.defineProperty(this, "directiveText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "btnSpeed", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "btnRestart", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "speedIdx", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 1
        ...
        Object.defineProperty(this, "winRunId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "endDelay", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "deadGrace", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "directive", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "directiveModMap", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {}
        ...
        Object.defineProperty(this, "locking", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        this.directiveText.style.wordWrap = true;
        this.directiveText.style.align = 'center';
    ...
    onUpdate(dt) {
        ...
        if (this.deadGrace > 0)
            this.deadGrace -= dt;
        if (!this.winner || this.resolved)
            return;
        if (this.winRunId !== this.runId) {
            this.winner = null;
            return;
        }
        this.endDelay -= dt;
    ...
    async ensureDirectiveForStage(stage) {
        ...
        if (this.game.state.directorStage === stage && this.game.state.directorDirective) {
            return this.game.state.directorDirective;
        }
    ...
    buildPlayerTeam(heroes) {
        return heroes.map((o, i) => {
            ...
            return fighter(`p${i + 1}:${o.heroId}`, def?.name ?? `英雄${i + 1}`, 'A', this.applyMult(stats.hp, hpMult), this.applyMult(stats.atk, atkMult), stats.def, this.applyMult(stats.spd, spdMult), def?.skills ?? [], def?.element);
        });
    }
    buildEnemyTeam(heroes) {
        ...
        const avgLv = Math.max(1, Math.round(heroes.reduce((s, h) => s + (h.level || 1), 0) / heroes.length));
        if (conf) {
            ...
            const teamB = conf.enemies.map((e, i) => {
                ...
                return fighter(`e${i + 1}`, e.name, 'B', this.applyMult((s.hp * m) | 0, hpMult), this.applyMult((s.atk * m) | 0, atkMult), (s.def * m) | 0, this.applyMult(s.spd, spdMult), e.skills ?? [], e.element ?? ELEMENTS[i % ELEMENTS.length]);
            });
        ...
        const teamB = Array.from({ length: cnt }, (_, i) => {
            ...
            return fighter(`e${i + 1}`, `敌人${i + 1}`, 'B', this.applyMult((s.hp * m) | 0, hpMult), this.applyMult((s.atk * m) | 0, atkMult), (s.def * m) | 0, this.applyMult(s.spd, spdMult), pool[i % pool.length], ELEMENTS[i % ELEMENTS.length]);
        });
    ...
    settle(winner) {
        ...
        // ── 标题 ──
        const t = createText(isWin ? '🎉 胜利！' : winner === 'B' ? '💀 失败' : '⚔ 平局', 36, 0xffffff, '900');
        t.anchor.set(0.5);
        ...
        // 计算归一化最大值（两队合并）
        const maxStats = {};
        for (const key of ['damageDealt', 'damageTaken', 'healingDone', 'shielding']) {
    ...
    grantRewards(stage, isBoss, isElite) {
        const heroes = this.game.state.getPartyHeroes().slice(0, 5)
            .map(id => this.game.state.getOwnedHero(id)).filter(Boolean);
        const valid = heroes.filter((h) => HERO_MAP[h.heroId]);
        const avgLv = valid.length > 0
            ? Math.max(1, Math.round(valid.reduce((s, h) => s + (h.level || 1), 0) / valid.length))
            : 1;
        const lines = [];
        ...
        if (goldMult)
            gold = Math.max(1, this.applyMult(gold, goldMult));
        ...
        this.game.state.withBatch(() => {
            ...
            if (isBoss || shrBonus > 0 || chestBonus > 0) {
                ...
                if (chestKey)
                    inv[chestKey] = (inv[chestKey] || 0) + (isBoss ? 1 : 0) + chestBonus;
            }
            if (Object.keys(inv).length > 0)
                this.game.state.applyInventoryDeltas(inv);
            // Stage progress
    ...
    rollChest(stage) {
        let r = Math.random();
        let key = 'chest_c';
        for (const c of BattleScene.CHEST_PROB) {
            r -= c.p;
            if (r <= 0) {
                key = c.key;
                break;
        ...
        if (stage % 50 === 0 && (key === 'chest_c' || key === 'chest_b'))
            key = 'chest_a';
        const NAMES = {
            chest_c: '普通宝箱', chest_b: '高级宝箱',
            chest_a: '史诗宝箱', chest_s: '传说宝箱',
        };
        return { key, name: ECONOMY[`${key}Name`] ?? NAMES[key] ?? key };
    }
}
Object.defineProperty(BattleScene, "SPEEDS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: [0.75, 1, 1.5, 2]
});
Object.defineProperty(BattleScene, "CHEST_PROB", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: [
