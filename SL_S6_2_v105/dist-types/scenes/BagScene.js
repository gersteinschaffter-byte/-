...
export default class BagScene extends BaseScene {
    ...
    openChestModal(chestKey, chestName) {
    ...
    openChestOnce(chestKey, chestName) {
    ...
    drawList() {
        ...
        const items = [
            ...
            // Boss chests
            { key: 'chest_c', name: ECONOMY.chest_cName ?? '普通宝箱', icon: '📦' },
            { key: 'chest_b', name: ECONOMY.chest_bName ?? '高级宝箱', icon: '🎁' },
            { key: 'chest_a', name: ECONOMY.chest_aName ?? '史诗宝箱', icon: '🟣' },
            { key: 'chest_s', name: ECONOMY.chest_sName ?? '传说宝箱', icon: '🟡' },
        ];
        let y = 60;
        for (const it of items) {
            ...
            if (this.isChestKey(it.key)) {
                ...
                if (q > 0)
                    row.on('pointertap', () => this.openChestModal(it.key, it.name));
            }
