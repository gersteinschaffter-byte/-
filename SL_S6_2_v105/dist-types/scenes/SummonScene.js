...
import SummonResultPopup, {} from './summon/SummonResultPopup';
...
export default class SummonScene extends BaseScene {
    ...
    async trySummon(count) {
        ...
        try {
            ...
            const results = this.game.state.withBatch(() => {
                const arr = [];
                for (let i = 0; i < count; i++)
                    arr.push(this.rollOne());
                return arr;
            });
            // Log results (hero ids + new/dupe) for bug reports.
            try {
                this.game.debug.info('SUMMON', 'result', results.map((r) => ({ id: r.hero.id, rarity: r.hero.rarity, isNew: r.isNew })));
            }
            catch (_) { }
            // Show popup (always in PopupLayer)
            if (count === 1)
                this.popup.showSingle(results[0]);
            else
                this.popup.showMulti(results);
        }
        finally {
            this.busy = false;
            this.refresh();
    ...
    spendSummonCost(count) {
        return this.game.state.withBatch(() => {
            ...
            if (!this.game.state.trySpendDiamonds(dia)) {
                ...
                if (useTickets > 0)
                    this.game.state.addInventory(pool.ticketKey, useTickets);
