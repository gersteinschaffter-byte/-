...
class HeroesScene extends BaseScene {
    constructor(game) {
        ...
        Object.defineProperty(this, "_onHeroesChanged", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: () => this.scheduleRefresh('heroesChanged')
        });
        Object.defineProperty(this, "initialGridLoaded", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "initialGridTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "skeletonNodes", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "pendingTimeouts", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Set()
        ...
        Object.defineProperty(this, "heroGridOrder", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "refreshQueued", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "heroModalRefs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "partyText", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "partyHint", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "btnFormation", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "scrollMask", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "pointerDown", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "lastMoveY", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "scrollStartY", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        ...
        Object.defineProperty(this, "scrollBaseX", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 16
        });
        Object.defineProperty(this, "scrollBaseY", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 16
        ...
        Object.defineProperty(this, "scrollFriction", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0.93
        ...
        Object.defineProperty(this, "scrollThumb", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "_scrollIndicatorThrottle", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        this.panel.on('pointermove', (e) => {
            ...
            if (!this.isDragging)
                return;
            // ★ v0.84: 修复坐标系 — e.global 是屏幕像素，scroll.y 是虚拟坐标
            // 需要除以 world.scale 来转换
            const worldScale = this.game.world.scale.x || 1;
            const dyVirtual = dy / worldScale;
            // Apply scroll (absolute from touch-start) for stability.
            this.scroll.y = this.scrollStartY + dyVirtual;
            // Velocity: use step delta in virtual coords
            const stepScreen = e.global.y - this.lastMoveY;
            const stepVirtual = stepScreen / worldScale;
            this.scrollVelocity = clamp(stepVirtual, -120, 120);
            this.lastMoveY = e.global.y;
            this.clampScroll();
    ...
    onExit() {
        ...
        if (this.initialGridTimer) {
            try {
                clearTimeout(this.initialGridTimer);
            }
            catch (_) { }
    ...
    scheduleRefresh(reason) {
        ...
        queueMicrotask(() => {
            ...
            if (PERF_DEBUG)
                this.perfMark(`refresh(${reason})`, start);
            this.refresh();
        });
    }
    onResize(w, _h) {
        if (!this.title || this.title.destroyed)
            return;
        this.title.position.set(w / 2, 170);
        if (this.partyText && !this.partyText.destroyed) {
            this.partyText.position.set(w / 2, 210);
        }
        if (this.partyHint && !this.partyHint.destroyed) {
            this.partyHint.position.set(w / 2, 242);
        }
        if (this.btnFormation && !this.btnFormation.destroyed) {
    ...
    layoutGrid() {
        ...
        if (!this.initialGridLoaded) {
            ...
            if (PERF_DEBUG)
                this.perfMark('layoutGrid(skeleton)', t0);
            return;
        }
        ...
        const ownedMap = new Map(state.heroes.map((h) => [h.heroId, h]));
        const list = HEROES.slice();
        const rarityRank = (r) => (r === RARITY.SP ? 4 : r === RARITY.SSR ? 3 : r === RARITY.SR ? 2 : 1);
        list.sort((a, b) => {
        ...
        for (let i = 0; i < newOrder.length; i++) {
            ...
            if (card)
                this.scroll.setChildIndex(card, i);
        }
        this.heroGridOrder = newOrder;
        this.contentHeight = maxY;
        this.updateScrollIndicator();
        if (PERF_DEBUG)
            this.perfMark('layoutGrid(buildCards)', t0);
    }
    renderSkeletonGrid() {
    ...
    clampScroll(forceTop = false, updateIndicator = false) {
...
${statsLine}`, element, skills, counter };
    ...
    openHeroModal(heroId) {
        ...
        if (PERF_DEBUG)
            this.perfMark('openHeroModal', t0);
    }
    updateHeroModal(heroId) {
        ...
        const starCostMap = { 1: 20, 2: 50, 3: 100, 4: 200 };
        ...
        if (refs.costHintText) {
            if (!ownedOk)
                refs.costHintText.text = '获得后可升级（消耗金币）';
            else
                refs.costHintText.text = `升级消耗：🪙 ${ECONOMY.levelUpGoldBase + Math.floor((owned.level - 1) * 18)}`;
        }
        ...
        if (PERF_DEBUG)
            this.perfMark('updateHeroModal', t0);
    }
    ensureHeroModalBuilt() {
        // Global modal is shared across scenes/popups.
        // We render hero details in an isolated modal layer to avoid cross-popup UI wipes.
        const modal = this.game.modal;
        const layer = modal.useLayer(PopupLayers.HERO_DETAIL, false);
        if (this.heroModalRefs) {
            // With namespaced layers, cached refs remain attached; no need to rebuild every time.
            return;
        }
        ...
        layer.addChild(header, cardSlot, bodyPanel, footer);
        ...
        modal.onClose = () => {
            ...
            if (this.heroModalRefs)
                this.heroModalRefs.unsubs = [];
            modal.onClose = prevOnClose;
            prevOnClose?.();
            // Drop cached refs: the modal is shared and other popups may rebuild its content.
            this.heroModalRefs = null;
        };
        this.heroModalRefs = {
            heroId: '',
            statValueTexts: {
    ...
    updateHeroModalCard(heroId) {
        ...
        if (!hero)
            return;
        if (!refs.card || refs.card.hero?.id !== heroId) {
        ...
        if (refs.cardTextRefs) {
            ...
            if (raw && refs.cardTextRefs.nameLine.width > maxNameW) {
                ...
                while (t.length > 1) {
                    ...
                    if (refs.cardTextRefs.nameLine.width <= maxNameW)
                        break;
                }
            }
        }
    }
    updateHeroModalSkills(detail) {
        const refs = this.heroModalRefs;
        if (!refs?.skillItems || !refs.scroll || !refs.scrollW)
            return;
    ...
    refreshModalButtons(heroId) {
        const refs = this.heroModalRefs;
        if (!refs || !refs.btnParty || !refs.btnLevel || !refs.btnStar)
            return;
        ...
        const partyCountNow = this.game.state.getPartyHeroes().length;
        refs.btnParty.setLabel(inPartyNow ? '下阵' : '上阵');
        if (!ownedNow) {
            // Not owned: all action buttons should be disabled to avoid confusing taps.
            refs.btnParty.setLabel('未拥有');
            refs.btnParty.setDisabled(true);
            refs.btnLevel.setLabel('升级');
            refs.btnLevel.setDisabled(true);
            refs.btnStar.setLabel('升星');
    ...
    handleToggleParty() {
        ...
        if (PERF_DEBUG)
            this.perfMark('toggleParty(state)', tState);
        if (!res.ok) {
        ...
        this.updateHeroModal(heroId);
        if (PERF_DEBUG)
            this.perfMark('toggleParty(ui)', tUi);
        if (PERF_DEBUG)
            this.perfMark('toggleParty(total)', t0);
    }
    handleLevelUp() {
        const heroId = this.heroModalRefs?.heroId;
        if (!heroId)
        ...
        if (PERF_DEBUG)
            this.perfMark('levelUp(state)', tState);
        if (!res.ok) {
    ...
    handleStarUp() {
        ...
        const res = this.game.state.tryStarUpHero(heroId, shardKey, cost, maxStars);
        if (PERF_DEBUG)
            this.perfMark('starUp(state)', tState);
        if (!res.ok) {
            this.game.toast.show(res.reason ?? '升星失败', 2);
            return;
        }
        this.game.toast.show(`升星成功：${res.stars ?? (curStars + 1)}★`, 2);
        const tUi = PERF_DEBUG ? this.perfNow() : 0;
        this.updateHeroModal(heroId);
        this.scheduleRefresh('starUp');
        if (PERF_DEBUG)
            this.perfMark('starUp(ui)', tUi);
    }
    // Phase 2: refreshed by subscriptions.
    refresh() {
        const t0 = PERF_DEBUG ? this.perfNow() : 0;
        const partyCount = Math.max(0, this.game.state.getPartyHeroes().length);
        if (this.partyText && !this.partyText.destroyed) {
        ...
        this.clampScroll(true, true);
        if (PERF_DEBUG)
            this.perfMark('refresh(total)', t0);
    }
    /**
...
Object.defineProperty(HeroesScene, "DRAG_THRESHOLD", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 14
