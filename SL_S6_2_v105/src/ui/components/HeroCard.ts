...
/** 稀有度等级 → 发光层数和强度 */
function rarityGlowConfig(rarity: string): { layers: { expand: number; alpha: number; lineW: number }[] } {
  switch (rarity) {
    case RARITY.SP:
      return {
        layers: [
...
export default class HeroCard extends Container {
  ...
  // ── 图层字段 ──
  private readonly shadowLayer: Graphics;        // Layer 0
  private readonly neonGlow: Graphics;           // Layer 1
  private readonly bodySprite: Sprite;           // Layer 2
  private readonly bg: Graphics;                 // Layer 3 (边框)
  private readonly cornerDeco: Graphics;         // Layer 3.5
  private readonly portrait: Container;          // Layer 4
  private readonly barSprite: Sprite;            // Layer 5
  private readonly bottomBar: Graphics;          // Layer 5 fallback
  private readonly nameTxt: Text;                // Layer 6
  private readonly subTxt: Text;                 // Layer 6
  private readonly rarityRibbon: Container;      // Layer 7
  private readonly rarityRibbonBg: Graphics;
  private readonly rarityRibbonTxt: Text;
  private readonly elementBadge: Container;      // Layer 7
  private readonly elementBadgeBg: Graphics;
  private readonly elementBadgeTxt: Text;
  public readonly glow: Graphics;                // Layer 8 (外部可控)
  private readonly lockOverlay: Container;       // Layer 9
  private readonly lockBg: Graphics;
  private readonly lockTxt: Text;
  private readonly partyBadge: Container;        // Layer 9
  private readonly partyBadgeBg: Graphics;
  private readonly partyBadgeTxt: Text;

  // Texture 缓存
  private bodyTex: Texture | null = null;
  private barTex: Texture | null = null;
  ...
  constructor(hero: HeroDef, owned?: OwnedHero) {
    ...
    this.bodyTex = makeCardBodyTexture(
      this.w, this.h,
      lighten(0x0e1733, 0.08),  // 顶部稍亮
