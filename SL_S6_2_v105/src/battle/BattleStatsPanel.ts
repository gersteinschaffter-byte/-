...
import type { FighterStats } from './BattleStats';
...
interface StatCategory {
  label: string;
  key: 'damageDealt' | 'damageTaken' | 'healingDone' | 'shielding';
  color: number;
}

const STAT_CATS: StatCategory[] = [
...
export function buildStatsPanel(opts: {
  ...
  teamA: FighterStats[];
  teamB: FighterStats[];
  mvp: FighterStats | null;
  maxStats: Record<string, number>;  // key → max value for normalization
}): Container {
...
function drawGradeRow(fighter: FighterStats, totalW: number, teamMaxScore: number): Container {
  const row = new Container();
