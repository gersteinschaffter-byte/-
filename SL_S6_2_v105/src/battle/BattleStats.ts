...
export interface FighterStats {
  ...
  damageDealt: number;     // 伤害输出
  damageTaken: number;     // 承受伤害
  healingDone: number;     // 治疗量
  shielding: number;       // 护盾贡献（估算值）

  kills: number;           // 击杀数
  buffGiven: number;       // 施加增益次数
  survived: boolean;       // 是否存活

  mvpScore: number;        // MVP 评分（由 evaluate 计算）
  mvpGrade: string;        // S/A/B/C 评级
}
...
/** 护盾 buff 的估算伤害减免值 */
const SHIELD_BUFF_VALUE: Record<string, number> = {
...
export class BattleStatsCollector {
  private fighters = new Map<string, FighterStats>();
  private lastSkillSource: string | null = null;

  /** 重置，开始新一场战斗 */
  reset(teamA: FighterSnapshot[], teamB: FighterSnapshot[]): void {
    this.fighters.clear();
    ...
    switch (e.type) {
      ...
      case 'damage': {
        ...
        if (sourceId !== targetId) {
          ...
          if (src) src.damageDealt += amount;
        }
        const tar = this.fighters.get(targetId);
        if (tar) tar.damageTaken += amount;
        break;
      }
      ...
      case 'shield': {
        const src = this.fighters.get(e.payload.sourceId);
        if (src) src.shielding += e.payload.amount;
        break;
      }
      ...
      case 'dead': {
        ...
        if (tar) tar.survived = false;
        // 最后一个技能使用者获得击杀
        if (this.lastSkillSource) {
          const killer = this.fighters.get(this.lastSkillSource);
          if (killer && killer.id !== e.payload.targetId) killer.kills++;
        }
        break;
      }
    }
  ...
  /** 获取某阵营数据（按 MVP 分降序） */
  getTeam(side: Side): FighterStats[] {
    const arr: FighterStats[] = [];
    for (const f of this.fighters.values()) {
      if (f.side === side) arr.push(f);
    }
    return arr.sort((a, b) => b.mvpScore - a.mvpScore);
  }

  /** 获取全场 MVP（我方优先） */
  getMVP(): FighterStats | null {
    const a = this.getTeam('A');
    return a[0] ?? this.getTeam('B')[0] ?? null;
  }

  /** 获取某个统计维度的最大值（用于条形图归一化） */
  getMaxStat(side: Side, key: 'damageDealt' | 'damageTaken' | 'healingDone' | 'shielding'): number {
    let max = 0;
    for (const f of this.fighters.values()) {
      if (f.side === side && f[key] > max) max = f[key];
    }
