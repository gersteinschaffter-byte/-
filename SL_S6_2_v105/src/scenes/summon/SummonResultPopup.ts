...
import ParticleSystem, { SummonBurst, RarityBurst, CardSparkle } from '../../fx/ParticleSystem';
...
export default class SummonResultPopup {
  ...
  /** Show a single hero result (with big card + glow + ★ particle burst). */
  public showSingle(item: SummonResultItem): void {
    const modal = this.game.modal;
    const layer = modal.useLayer(PopupLayers.SUMMON_RESULT);
    ...
    // ★ v0.83: 粒子系统 — 根据稀有度选择爆发效果
    const ps = new ParticleSystem({ maxParticles: 80 });
    layer.addChild(ps);

    const isHighRarity = item.hero.rarity === RARITY.SSR || item.hero.rarity === RARITY.SP;
    const cardCenterX = (panelW - card.w * 1.32) / 2 + (card.w * 1.32) / 2;
    const cardCenterY = 160 + (card.h * 1.32) / 2;

    // 延迟一点点再爆发（配合入场感）
    let burstDelay = 8;
    let sparkleInterval = 0;
    const SPARKLE_RATE = isHighRarity ? 25 : 45; // 高稀有度更频繁闪光

    // Subtle glow pulse + ★ particle drive
    let t = 0;
    ...
    const tick = (dt: number) => {
      ...
      // ★ 初始爆发
      burstDelay -= dt;
      if (!burstFired && burstDelay <= 0) {
        burstFired = true;
        const burstPreset = isHighRarity ? RarityBurst : SummonBurst;
        const burstCount = isHighRarity ? 45 : 25;
        ps.burst(burstPreset, cardCenterX, cardCenterY, burstCount);
      }

      // ★ 持续微光闪烁（高稀有度卡周围）
      sparkleInterval += dt;
      if (burstFired && sparkleInterval >= SPARKLE_RATE) {
        sparkleInterval = 0;
        ps.burst(CardSparkle, cardCenterX, cardCenterY, isHighRarity ? 3 : 1);
      }
    ...
    layer.addChild(title, card, meta, tagBox, note, btnOk);
    modal.onClose = () => {
      this.unbindWheel?.();
      this.unbindWheel = undefined;
      this.game.pixi.ticker.remove(tick);
      ps.clearAll();
      modal.clearLayer(PopupLayers.SUMMON_RESULT);
    };
    modal.open();
  }
  ...
  public showMulti(items: SummonResultItem[]): void {
    ...
    layer.addChild(title, hint, list, btnOk);
    modal.onClose = () => {
