...
import type { BattleFXManager } from './BattleFX';
...
const ELEM: Record<string, { icon: string; tint: number }> = {
...
export default class FighterNode {
  ...
  public baseY = 0;
  public readonly phase = Math.random() * Math.PI * 2;
  ...
  // 视觉层
  private readonly glowActive = new Graphics();
  private readonly glowTurn   = new Graphics();
  private readonly body       = new Graphics();
  private readonly bodySheen  = new Graphics();
  private readonly topStrip   = new Graphics();   // ❶ 顶部阵营色条
  private readonly hpBar      = new Graphics();
  private readonly hpLabel: Text;
  private readonly nameLabel: Text;

  // Buff 栏
  private readonly buffBar = new Container();
  private readonly buffs   = new Map<string, BuffNode>();
  ...
  setActive(active: boolean): void {
    if (this.destroyed || this.isDead) { this.glowActive.visible = false; return; }
    this.glowActive.visible = active;
    if (!active) return;
    this.paintGlow(this.glowActive, 10, 0xfff4a0, 0.08, 0.35);
  }

  flashTurn(runner: TweenRunner): void {
    ...
    runner.add(Tween.to(this.glowTurn, { alpha: 0 }, 20, easeOutCubic, () => {
      if (!this.destroyed) this.glowTurn.clear();
    }));
  }

  // ── 颜色闪烁 ──

  flashColor(color: number, runner: TweenRunner): void {
    this.spawnOverlayFlash(color, 0.35, 14, runner);
  }

  flashPoison(runner: TweenRunner): void {
    if (this.destroyed) return;
    const ov = this.createOverlay(0xcc44ff, 0.28);
    ov.alpha = 0;
    this.container.addChild(ov);
    const blink = (n: number): void => {
      if (n <= 0 || this.destroyed) { this.safeRemove(ov); return; }
  ...
  applyShield(amount: number, shield: number, maxShield: number, fx: BattleFXManager): void {
    ...
    fx.shieldParticles(this.x, this.y);
  }

  playDeath(fx: BattleFXManager, runner: TweenRunner): void {
  ...
  playAttack(tx: number, ty: number, fx: BattleFXManager, color = 0xffffff): void {
    fx.attackLine(this.x, this.y, tx, ty, color);
  }

  // ── Buff 图标 ──

  addBuff(buffId: string, icon: string, duration?: number): void {
    if (this.buffs.has(buffId)) return;
  ...
  updateBuffTurns(buffId: string, turns?: number): void {
    const b = this.buffs.get(buffId);
    if (!b?.turns || turns == null || !Number.isFinite(turns)) return;
  ...
  private paintGlow(g: Graphics, pad: number, color: number, fillA: number, strokeA: number): void {
    g.clear();
    const x = -Box.HW - pad, y = -Box.HH - pad;
    const w = Box.W + pad * 2, h = Box.H + pad * 2;
    const r = Box.R + pad / 2;
    if (fillA > 0) { g.beginFill(color, fillA); g.drawRoundedRect(x, y, w, h, r); g.endFill(); }
    if (strokeA > 0) { g.lineStyle(3, color, strokeA); g.drawRoundedRect(x, y, w, h, r); }
  }

  private layoutBuffs(): void {
    let x = 0;
    for (const b of this.buffs.values()) { b.root.position.set(x, 0); x += 26; }
  }

  private createOverlay(color: number, alpha: number): Graphics {
  ...
  private spawnOverlayFlash(color: number, alpha: number, frames: number, runner: TweenRunner): void {
    if (this.destroyed) return;
  ...
  private safeRemove(g: Graphics): void {
    if (!this.destroyed) this.container.removeChild(g);
    g.destroy();
  }
}
