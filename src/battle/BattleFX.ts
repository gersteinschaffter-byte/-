...
export class BattleFXManager {
  ...
  skillHitParticles(skillId: string, x: number, y: number): void {
    ...
    // v113: 同步触发 sprite-sheet 特效，避免“只有粒子、看起来像没生效”。
    if (skillId.includes('heal') || skillId.includes('revitalize') || skillId.includes('haste')) {
      this.playSprite('heal_ring', x, y, { scale: 1.2, speed: 0.55, blendAdd: true });
      return;
    }
    if (skillId.includes('shield') || skillId.includes('aegis') || skillId.includes('barrier') || skillId.includes('ironwall')) {
      this.playSprite('shield_pop', x, y, { scale: 1.25, speed: 0.55, blendAdd: true, alpha: 0.95 });
      return;
    }

    this.playSprite('hit_spark', x, y, { scale: 1.05, speed: 0.68, blendAdd: true, alpha: 0.95 });
  }
...
shieldParticles(x: number, y: number): void {
  ...
  void loadFxSheet().then((s) => { this.spriteSheet = s; }).catch(() => { /* fallback */ })
    .finally(() => { this.spriteSheetLoading = false; });
}

  private playSprite(key: 'slash_arc'|'hit_spark'|'shield_pop'|'heal_ring', x: number, y: number, opts?: { scale?: number; speed?: number; alpha?: number; blendAdd?: boolean }): void {
